...
return data
    ...
    logging.error(f "File {file_path} is corrupted. Resetting the file.")
...
self.running = False
self.counter = 0
self.crack_attempts = 0# Add a counter
for crack attempts
...
reported = self.report.data_field_or('reported',
    default = [])
excluded = self.report.data_field_or('excluded',
    default = [])
...
# Load logs
for checked Wi - Fi networks and crack attempts
checked_wifis = load_json_file(os.path.join(LOG_DIR, 'checked_wifis.json'))
crack_attempts = load_json_file(os.path.join(LOG_DIR, 'crack_attempts.json'))
successful_cracks = load_json_file(os.path.join(LOG_DIR, 'successful_cracks.json'))
...
def check_handshake(self, filename): #Convert.pcap file to.22000 format
...
# Execute hashcat to check
if the handshake is crackable
...
atomic_write(log_file_path, [])
checked_wifis = []

# Log the checked Wi - Fi network only
if it hasn 't been logged before
if filename not in [wifi["filename"]
        for wifi in checked_wifis
    ]:
    log_checked_wifi(filename, result)
checked_wifis.append({
    "filename": filename,
    "result": result
})
...
crack_attempts = load_json_file(log_file_path)

# Log the crack attempt only
if it hasn 't been logged before
crack_attempt_entry = {
    "filename": filename,
    "essid": essid,
...
if crack_attempt_entry not in crack_attempts:
    update_json_file(log_file_path, crack_attempt_entry)
crack_attempts.add(crack_attempt_entry)

if result: #Ensure the successful_cracks.json file exists
log_file_path = os.path.join(LOG_DIR, 'successful_cracks.json')
ensure_json_file_exists(log_file_path)

# Load existing successful cracks to avoid duplicates
successful_cracks = load_json_file(log_file_path)

# Log the successful crack only
if it hasn 't been logged before
successful_crack_entry = {
    "filename": filename,
    "essid": essid,
    "bssid": bssid,
...
if successful_crack_entry not in successful_cracks:
    update_json_file(log_file_path, successful_crack_entry)
